...
export async function mergeTiles(parts: Uint8Array[]): Promise<Uint8Array> {
  ...
  for (const [name, layerGroup] of layersByName.entries()) {
    ...
    layerGroup.sort((a, b) => (a._tileIndex ?? 0) - (b._tileIndex ?? 0))
    ...
    for (const lyr of layerGroup) {
      ...
      const scale = needRescale
        ? targetExtent / (lyr.extent ?? targetExtent)
        : 1
...
function parseLayer(layer: Uint8Array): ParsedLayer {
  ...
  while (i < layer.length) {
    ...
    if (wt === 2) {
      ...
      if (fn === 15) {
        version = Number(r.value)
      } else if (fn === 5) {
        extent = Number(r.value)
      }
      i = r.next
    } else if (wt === 5) {
      i += 4
  ...
  while (i < fb.length) {
    ...
    if (wt === 2) {
      ...
      if (fn === 2) {
        ...
        geometryBytes = new Uint8Array(fb.slice(i2, end))
      }
      i = end
    } else if (wt === 0) {
...
function parseValue(vb: Uint8Array): ValueUnion {
  ...
  while (i < vb.length) {
    ...
    if (fn === 2 && wt === 5) {
      // float (32-bit)
      const v = new DataView(vb.buffer, vb.byteOffset + i, 4).getFloat32(
        0,
        true,
      )
      i += 4
    ...
    if (fn === 3 && wt === 1) {
      // double (64-bit)
      const v = new DataView(vb.buffer, vb.byteOffset + i, 8).getFloat64(
        0,
        true,
      )
      i += 8
    ...
    if (fn === 4 && wt === 0) {
      const r = decodeVarint64(vb, i)
      i = r.next
      return {kind: "int", value: r.value}
    }
    if (fn === 5 && wt === 0) {
      const r = decodeVarint64(vb, i)
      i = r.next
      return {kind: "uint", value: r.value}
    }
    if (fn === 6 && wt === 0) {
      const r = decodeVarint64(vb, i)
      i = r.next
      return {kind: "sint", value: zigzagDecode64(r.value)}
    }
    if (fn === 7 && wt === 0) {
      const r = decodeVarint64(vb, i)
      i = r.next
      return {kind: "bool", value: r.value !== 0n}
    }

    // skip unknown
    if (wt === 2) {
      const {value: L, next} = decodeVarint32(vb, i)
      i = next + L
    } else if (wt === 0) {
      const r = decodeVarint64(vb, i)
      i = r.next
  ...
  while (i < geom.length) {
    ...
    if (cmd === 1) {
      ...
      for (let k = 0; k < count; k++) {
        const dxVar = decodeVarint32(geom, i)
        i = dxVar.next
        const dyVar = decodeVarint32(geom, i)
        i = dyVar.next
...
function rescaleGeometry(commands: Command[], scale: number): Command[] {
  ...
  return commands.map((c) => {
    ...
    const pts = c.pts.map(
      ([px, py]) =>
        [Math.round(px * scale), Math.round(py * scale)] as [number, number],
    )
...
function encodeGeometry(commands: Command[]): Uint8Array {
  ...
  const emitCmd = (id: number, count: number) =>
    out.push((count << 3) | (id & 0x7))
  const emitParam = (v: number) =>
    out.push(...encodeVarint32(zigzagEncode32(v)))
...
function encodeLayer(l: {
  ...
  chunks.push(encodeFieldVarint(15, l.version))
  total += chunks[chunks.length - 1].length

  // required string name = 1;
  chunks.push(encodeFieldString(1, l.name))
  total += chunks[chunks.length - 1].length
  ...
  chunks.push(encodeFieldVarint(5, l.extent))
  total += chunks[chunks.length - 1].length
...
function encodeFeature(f: EncodedFeature): Uint8Array {
  ...
  if (typeof f.id !== "undefined") {
    chunks.push(
      encodeFieldVarint64(1, typeof f.id === "bigint" ? f.id : BigInt(f.id)),
    )
    total += chunks[chunks.length - 1].length
  }
  ...
  chunks.push(encodeFieldBytes(2, tagsPacked))
  total += chunks[chunks.length - 1].length

  // optional GeomType type = 3
  chunks.push(encodeFieldVarint(3, f.type))
  total += chunks[chunks.length - 1].length

  // repeated uint32 geometry = 4 [packed = true]
  chunks.push(encodeFieldBytes(4, f.geometryBytes))
  total += chunks[chunks.length - 1].length
...
function encodeValue(v: ValueUnion): Uint8Array {
  ...
  if (v.kind === "float") {
    ...
    new DataView(b.buffer).setFloat32(0, v.value, true)
...
function zigzagEncode64(n: bigint): bigint {
  return (n << 1n) ^ (n >> 63n)
}
...
function canonicalValueKey(v: ValueUnion): string {
  switch (v.kind) {
    ...
    case "uint":
      return `u:${v.value.toString()}`
    case "sint":
      return `n:${v.value.toString()}`
    case "bool":
