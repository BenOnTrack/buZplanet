<script lang="ts">
	...
	// Toggle all categories in a class (filtered)
	...
	// Toggle all categories in a subclass (filtered)
	...
	// Check if all categories in a class are selected (filtered)
	function isClassSelected(className: string): boolean {
		const classCategories = Object.values(categoryHierarchy[className]).flat();
		return classCategories.every((cat) => selectedCategories.includes(cat));
	}

	// Check if some categories in a class are selected (filtered)
	...
	// Check if all categories in a subclass are selected (filtered)
	...
	// Check if some categories in a subclass are selected (filtered)
...
<div class={clsx('filtered-category-manager', className)}>
				...
				{#each Object.entries(categoryHierarchy) as [className, subclasses]}
					<fieldset class="rounded-lg border border-gray-200 p-4">
						...
						<div class="mt-3 space-y-4">
							{#each Object.entries(subclasses) as [subClassName, categoryList]}
								<div class="border-l-2 border-gray-200 pl-4">
									...
									<div class="mb-2">
										...
										>
											...
											<span class="text-xs opacity-75">
												({categoryList.filter((cat) => selectedCategories.includes(cat))
