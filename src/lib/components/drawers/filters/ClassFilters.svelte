...
<div class="filter-section">
	<div class="filter-label">Filter by Class:</div>
	<div class="filter-buttons">
		{#each availableClasses as className}
			...
			<button
				...
				onkeydown={(e) => e.key === 'Enter' && toggleClassFilter(className)}
				class="filter-button {isSelected ? 'selected' : ''}"
				title="{className} ({count} features)"
				role="checkbox"
				aria-checked={isSelected}
			>
				<PropertyIcon key="class" value={className} size={16} />
				<span class="filter-text">{className}</span>
